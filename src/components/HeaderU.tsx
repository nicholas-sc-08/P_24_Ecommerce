...
export default function Header() {
    ...
    return (
        <AnimatePresence>
            <motion.header initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="shadow-[0px_1px_3px_0px_rgba(0,0,0,0.02),0px_0px_0px_1px_rgba(27,31,35,0.15)]">
                <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-4">
                    ...
                    <div className="flex justify-around w-55">
                        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                            <Button variant="default" className="cursor-pointer" onClick={() => setCartCount(cartCount + 1)}><ShoppingCart /> Cart ({cartCount})</Button>
                        </motion.div>
                        <ModeToggle />
                        <motion.div>
